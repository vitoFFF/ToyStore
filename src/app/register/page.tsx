    ...
    const handleRegister = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (data.user) {
            ...
            if (profileError) {
                ...
                toast.warning(t("auth.profileFailed"));
            } else {
    ...
    return (
        <div className="min-h-[calc(100svh-5rem)] flex items-center justify-center py-10 -mt-20 px-4">
            <div className="w-full max-w-md mx-auto space-y-8 bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] shadow-2xl shadow-primary/5 border border-border text-center relative overflow-hidden">
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-2 bg-gradient-to-r from-purple-500 via-pink-500 to-yellow-500" />
                ...
                <div className="flex flex-col items-center gap-4">
                    ...
                    <div className="space-y-1">
                        <h1 className="text-3xl font-extrabold tracking-tight">{t("auth.joinFun")}</h1>
                        <p className="text-muted-foreground">{t("auth.createAccountSubtitle", { brand: BRAND.name })}</p>
                    </div>
                </div>
                ...
                <div className="text-sm text-muted-foreground pt-2">
                    {t("auth.haveAccount")} <Link href="/login" className="text-primary font-bold hover:underline">{t("auth.logIn")}</Link>
                </div>
            </div>
        </div>
    );
